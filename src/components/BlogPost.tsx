...
const BlogPost = () => {
  ...
  useEffect(() => {
    const fetchPost = async () => {
      try {
        // Determine the API URL based on the selected language
        const apiUrl =
          language === 'ar'
            ? `https://strapi.gtmcd.com/api/arabic-blogposts?filters[documentId][$eq]=${documentId}&populate=*`
            : `https://strapi.gtmcd.com/api/gulftrade-blogposts?filters[documentId][$eq]=${documentId}&populate=*`;
        ...
        if (data.data.length > 0) {
          ...
          setError(t('blogPostNotFound') || 'Blog post not found');
        }
        setLoading(false);
      } catch (err) {
        setError(t('blogPostError') || 'Failed to fetch blog post');
        setLoading(false);
      }
    };
    if (documentId) fetchPost();
  }, [documentId, language, t]); // Re-run effect when documentId or language changes
  ...
  return (
    <>
      <Header />
      <div className={`min-h-screen bg-gradient-to-b pt-32 py-32 from-blue-50 to-white ${language === 'ar' ? 'rtl font-arabic' : ''}`}>
        <div className="container mx-auto px-4 py-12 lg:px-8">
          ...
          >
            ...
            {t('backToBlog') || (language === 'ar' ? 'العودة إلى المدونة' : 'Back to Blog')}
          </button>
          {post && (
              ...
              <div className={`mt-8 p-4 bg-blue-100 rounded-lg flex justify-between items-center ${language === 'ar' ? 'flex-row-reverse' : ''}`}>
                <div className={language === 'ar' ? 'text-right' : ''}>
                  <p className="text-sm text-blue-800">
                    {t('published') || (language === 'ar' ? 'نُشر في' : 'Published')}: {new Date(post.date).toLocaleDateString(language === 'ar' ? 'ar-AE' : 'en-US', {
                      year: 'numeric',
                      month: 'long',
                      day: 'numeric',
                    })}
